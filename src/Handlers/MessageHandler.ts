...
export default class MessageHandler {
  ...
  handleMessage = async (M: ISimplifiedMessage): Promise<void> => {
    ...
    if (!command)
      return void M.reply(
        `🐦 Ahh shit, you don't even know the command *${this.client.config.prefix}help* , well fine in that case you'll die here!`
      );
    const user = await this.client.getUser(M.sender.jid);
    if (user.ban) return void M.reply("You're banned from using the listed commands🙂..... Have a Nice Day🛐");
    const state = await this.client.DB.disabledcommands.findOne({
    ...
    if (!command.config?.dm && M.chat === "dm")
      return void M.reply("Commands cannot be used in direct messages *Send a message(preferably text message only) without a prefix to have a chat*");
    if (
      command.config?.modsOnly &&
      !this.client.config.mods?.includes(M.sender.jid)
    ) {
      return void M.reply(`🐦Ugly people can't use this command😂🛐!`);
    }
    if (command.config?.adminOnly && !M.sender.isAdmin)
      return void M.reply(
        `This command is not meant for members, this is an *Admin-Only* command😷!`
      );
    try {
  ...
  sendReconnectMessage = async (): Promise<void> => {
    const text = `Reconnected🙂...Now somebody's gonna fuck m🛐🔪`;
    await this.client.sendMessage(
      "263716564623-1628429288@g.us",
      text,
      MessageType.text
